...
[Build your own `Operating System`](https://github.com/codecrafters-io/build-your-own-x?tab=readme-ov-file#build-your-own-operating-system)
...
### Как грузится ОС c BIOS

Классический BIOS сам по себе ничего не знает о формате ядра ОС. Он делает очень простую вещь:
1. Загружает первый сектор диска (MBR, 512 байт) по адресу 0x7C00
2. Передаёт туда управление в 16-битном real mode

Дальше всё полностью на совести загрузчика.

#### Вариант 1: свой загрузчик
Вы можете:
* написать свой MBR/bootloader
* сами:
    * переключиться в protected / long mode
    * загрузить ядро с диска
    * передать ему управление

#### Вариант 2: Multiboot / Multiboot2 (самый популярный)
Если вы хотите, чтобы GRUB загрузил ваше ядро, то:
* ядро должно соответствовать Multiboot или Multiboot2
* GRUB:
    * работает под BIOS
    * грузит ELF
    * передаёт вам:
...
### Как грузится ОС c UEFI
UEFI — это уже мини-операционная система, а не просто прошивка.

Что даёт UEFI
* 32/64-битный protected/long mode сразу
* нормальные драйверы
* файловая система (FAT32)
* загрузка .efi-приложений

Как грузится ОС в UEFI
* UEFI загружает файл BOOTX64.EFI
* Это обычное PE-приложение
* Оно:
    * либо само является загрузчиком
    * либо сразу ядром

Multiboot здесь не используется вообще
...
#### кросс-binutils для x86_64-elf

Для 64 бит нужны инструменты:
* x86_64-elf-as
* x86_64-elf-ld
* (опционально x86_64-elf-gcc)
...
```
which x86_64-elf-as
# $HOME/os_dev/cross/bin/x86_64-elf-as
...
<summary>Из книги "Компьютерные системы. Архитектура и программирование [2022] Брайант Р. Э., О'Халларон Д. Р."</summary>
...
Когда оболочка загружала и запускала программу `hello` и когда программа `hello` отображала свое сообщение, ни та ни другая программа не обращалась напрямую к клавиатуре, диску или основной памяти. Для этого они пользовались услугами, предоставляемыми операционной системой. Операционную систему можно представить как некоторый слой программного обеспечения между прикладной программой и аппаратными средствами. Любые операции с аппаратными средствами прикладные программы должны выполнять через операционную систему.

Ядро операционной системы играет роль посредника между прикладными программами и аппаратными средствами.

Операционная система прежде всего должна отвечать двум основным требованиям: (1) защищать аппаратные средства от катастрофических действий вышедшей из-под контроля программы и (2) предоставлять приложениям простые и единообразные механизмы манипулирования сложными и часто сильно отличающимися низкоуровневыми аппаратными средствами. Для этого операционная система предлагает набор фундаментальных абстракций: **процессы, виртуальную память и файлы, виртуальная машина**. Файлы являются абстракцией устройств ввода/вывода, виртуальная память является абстракцией как основной памяти, так и дисковых устройств ввода/вывода, а процессы – абстракцией процессора, основной памяти и устройств ввода/вывода. 

Обсудим эти абстракции по очереди.

**Процесс** – это абстракция операционной системы, представляющая выполняемую программу. В одной и той же системе одновременно может выполняться множество процессов, и в то же время каждому процессу кажется, что только он пользуется аппаратными средствами. С точки зрения программы создается впечатление, что только она распоряжается процессором, основной памятью и устройствами ввода/вывода.

Реализация абстракции процесса требует тесного взаимодействия аппаратных и программных средств операционной системы.

Операционная система хранит всю информацию о состоянии, необходимую для правильного выполнения процесса. Состояние, известное как контекст, содержит такие сведения, как текущее значение счетчика команд PC, состоя­ние блока регистров и содержимое основной памяти. В любой конкретный момент времени в системе выполняется только один процесс. Когда опера­ционная система принимает решение передать управление некоторому другому процессу, она производит переключение контекста, запоминая контекст текущего процесса и восстанавливая контекст нового, после чего передает управление новому процессу. Новый процесс возобновляет выполнение точно с того места, в котором он был прерван.

**Потоки**

Обычно мы представляем себе процесс как единственный поток управления, однако в современных системах процесс может состоять из множества единиц выполнения, которые называют потоками выполнения (или нитями), каждый поток выполняется в контексте процесса, использует тот же программный код и глобальные данные, что и сам процесс.
...
**Виртуальная машина**  предоставляющая абстракцию всего компьютера, включая операционную систему, процессор и программы. Идея виртуальной машины была представлена компанией IBM еще в 1960-х годах, но лишь не так давно она стала привлекать особое внимание как способ управления компьютерами, которые должны запускать программы, разработанные для разных операционных систем.


```
Unix, Poslx и Standard Unix Specification

В шестидесятые годы прошлого столетия господствовали большие и сложные операцион-
ные системы, такие как OS/360, разработанная компанией IBM, и Multics, разработанная
...
смогла добиться широкого признания. Компания Bell Laboratories первоначально была од-
ним из партнеров, разрабатывавших проект Multics, но в 1969 году отказалась от участия в
этом проекте по причине его чрезмерной сложности и ввиду отсутствия положительных ре-
зультатов. Полученный при разработке системы отрицательный опыт сподвиг группу иссле-
дователей компании – Кена Томпсона (Ken Thompson), Денниса Ритчи (Dennis Ritchie), Дуга
...
исключительно на машинном языке. Многие из идей новой системы, такие как иерархиче-
ская файловая система и понятие командной оболочки как процесса пользовательского
...
гранный». – Прим. перев.). Ядро Unix было переписано на языке С в 1973 году, а сама опера-
ционная система была представлена широкой публике в 1974 году [93].
...
сторонников среди студентов и преподавателей различных университетов. Работа, ока-
завшая большое влияние на дальнейшее развитие, была выполнена в Калифорнийском
университете Беркли в конце семидесятых и в начале восьмидесятых годов, когда иссле-
дователи из Беркли добавили виртуальную память и сетевые протоколы в последователь-
ность выпусков, получивших название Unix 4.xBSD (Berkeley Software Distribution).

Осложнения возникли в середине восьмидесятых годов, когда производители опе-
рационной системы предприняли попытку выбрать собственные направления в разви-
тии, добавляя новые свойства, часто нарушающие совместимость с прежними версиями.
Чтобы пресечь эти сепаратистские тенденции, институт стандартизации IEEE (Institute
for Electrical and Electronics Engineers – Институт инженеров по электротехнике и элек-
тронике) возглавил усилия по стандартизации системы Unix. Позже Ричард Столлман
...
как интерфейс языка С для системных вызовов в Unix, командные оболочки и утили-
ты, потоки и сетевое программирование.
```

**Виртуальная память**

Виртуальная память – это абстракция, которая порождает в каждом процессе иллюзию, что лишь он один использует основную память. Каждый процесс имеет одно и то же представление о памяти, которое известно как его виртуальное адресное пространство. В системе Linux верхняя часть адресного пространства резервируется для программного кода и данных операционной системы, которые являются общими для всех процессов. В нижней части адресного прост­ранства находятся программный код и данные, принадлежащие процессам пользователей. Обратите внимание на тот факт, что адреса на схеме увеличиваются снизу вверх.


        +---------------------------------------------------+
        |     Виртуальная память ядра                       |  Память, недоступная пользовательскому коду 
        +---------------------------------------------------+  
        |                                                   |
        |  Пользовательский стек (создается при запуске)    |
        |                                                   |
        +---------------------------------------------------+
        |                                                   |
        +---------------------------------------------------+
        |                                                   |
        |  Отображаемая область памяти для разделяемых      |  Функция printf 
        |  библиотек                                        |
        |                                                   |
        +---------------------------------------------------+
        |                                                   |
        +---------------------------------------------------+
        |                                                   |
        |  Динамическая память (создается malloc)           |
        |                                                   |
        +---------------------------------------------------+
        |                                                   |  | 
        |  Данные, доступные для чтения/записи              |  |
        |                                                   |  | Загружаются из
        +---------------------------------------------------+  |
        |                                                   |  | выполняемого файла hello
        |  Код и данные, доступные только для чтения        |  |   
        |                                                   |  | 
        +---------------------------------------------------+    
        |  Начало программы → 0                             |   
        +---------------------------------------------------+
...
Виртуальное адресное пространство, с точки зрения каждого процесса, состоит из некоторого числа четко определенных областей, каждая из которых выполняет свою функцию.
* Программный код и данные. Программный код каждого процесса всегда начинается с одного и того же адреса, за ним следуют ячейки памяти, соответствующие глобальным переменным. Область с кодом и данными инициализируется непосредственно содержимым выполняемого объектного файла, в нашем случае hello.
...
* Совместно используемые (разделяемые) библиотеки. Ближе к середине адресного пространства находится область с программным кодом и данными совместно используемых библиотек, таких как стандартная библиотека С или библиотека математических функций.
* Стек. В верхней части виртуального адресного пространства находится стек пользователя, который применяется компилятором для реализации вызовов функций. Как и динамическая память, стек пользователя может динамически расширяться и сокращаться в размерах во время выполнения программы. В частности, каждый раз, когда программа вызывает какую-либо функцию, размер стека увеличивается. Каждый раз, когда функция возвращает управление, стек сокращается
...
Для правильной работы виртуальной памяти требуется сложное взаимо­действие аппаратных и программных средств операционной системы, включая аппаратное преобразование каждого адреса, генерируемого процессором. Основная идея заключается в том, чтобы сохранить содержимое виртуальной памяти процесса на диске, а затем использовать основную память как кеш диска.

---

### асм-вставки

Написание ядер ОС и драйверов

Когда ты пишешь операционную систему, тебе нужно управлять железом напрямую:

* Сбросить кэш процессора.

* Переключить таблицу страниц памяти.

* Разрешить или запретить прерывания. На Си это сделать невозможно физически — в языке просто нет слов для таких действий. Тут без asm вставок компьютер даже не загрузится.
...
<summary>Три основные альтернативные парадигмы организации операционной системы</summary>


Это очень глубокий и концептуальный вопрос, который лежит в основе различий между различными семействами операционных систем!

Вы абсолютно правы: в Linux (и в UNIX-подобных системах, включая macOS) действует философия "Всё есть файл". Это означает, что для ядра файл — это стандартизированный интерфейс (набор байтов, которые можно читать, писать и перемещать указатель), который используется для доступа ко всему: от файлов на диске до сетевых сокетов и даже системных ресурсов.

Однако существуют три **основные альтернативные парадигмы** организации операционной системы, которые не используют "файл" как универсальный интерфейс:



## 1. Объекты (Объектно-ориентированные ОС)

В этой парадигме основные системные ресурсы представляются как **абстрактные объекты** со строгими методами (функциями) для взаимодействия, а не как просто последовательность байтов.

* **Примеры:** Windows NT (внутренняя архитектура), NeXTSTEP, BeOS, а также ядро Mach (использовалось в ранних версиях macOS).
* **Концепция:** Вместо того чтобы открыть файл (дескриптор) для принтера и писать в него байты, вы получаете **объект "Принтер"** и вызываете его метод `Printer.PrintDocument(doc_object)`.
* **Преимущества:**
    * **Строгая типизация:** Интерфейсы взаимодействия строго определены, что снижает вероятность ошибок.
    * **Инкапсуляция:** Внутренняя логика ресурса (например, как работает принтер или сеть) скрыта от пользователя.
* **Недостатки:** Сложность взаимодействия, так как нет единого универсального интерфейса (как чтение/запись байтов).

## 2. Реестры и Атомарные Сущности (Windows NT)

Хотя Windows NT внешне имеет файловую систему, внутри она в значительной степени построена на специализированных, атомарных хранилищах данных и структурных элементах.

* **Пример:** Семейство Windows NT (XP, Vista, 7, 10 и т.д.).
* **Концепция:** Основная конфигурация системы, пользователя, оборудования и программного обеспечения хранится в **Реестре (Registry)** — иерархической базе данных, а не в конфигурационных файлах в `/etc`. Процессы, потоки и синхронизация управляются через строгие **объектные хэндлы** (Handle), которые не являются файлами.
* **Преимущества:**
    * **Централизация:** Упрощенное управление конфигурацией и состоянием системы.
    * **Транзакционность:** Реестр позволяет выполнять атомарные изменения (все или ничего), что делает систему более устойчивой к сбоям при обновлении конфигурации.
* **Недостатки:** Реестр может стать очень большим и подверженным повреждению. Универсальность UNIX-стиля теряется.

## 3. Сообщения и Микроядра (Message Passing & Microkernels)

Эта парадигма радикально отличается тем, что пытается максимально уменьшить код, работающий в режиме ядра (Kernel Mode).

* **Примеры:** MINIX, QNX (используется в автомобильных системах и промышленном оборудовании), Hurd (попытка заменить ядро Linux).
* **Концепция:** Ядро предоставляет лишь минимальный набор примитивов (управление памятью, планирование и **передача сообщений**). Все остальное (файловые системы, драйверы, сетевые стеки) выполняется в **режиме пользователя (User Mode)** как отдельные, независимые *серверы*. Взаимодействие между компонентами (например, приложению и драйверу диска) происходит исключительно через **синхронную или асинхронную передачу сообщений**. 
* **Преимущества:**
    * **Стабильность:** Сбой в драйвере (например, Wi-Fi) не приведет к падению всего ядра, так как драйвер работает в отдельном, защищенном пространстве.
    * **Модульность:** Систему очень легко модифицировать и отлаживать.
* **Недостатки:** Производительность может быть ниже из-за накладных расходов на постоянную передачу сообщений и переключение контекста между процессами.
 

 

## 4. Файловая Парадигма (UNIX/Linux)

В этой парадигме основные системные ресурсы (файлы на диске, устройства, потоки ввода-вывода) представляются как **поток байтов**, доступ к которому осуществляется через универсальный интерфейс — **файловый дескриптор**.

* **Примеры:** UNIX, Linux, BSD, Solaris.
* **Концепция:** К любому ресурсу можно получить доступ с помощью четырех основных системных вызовов: `open()`, `read()`, `write()`, и `close()`. Это называется **Универсальностью интерфейса**.
* **Реализация:** Драйверы устройств и сетевые соединения "маскируются" под файлы, расположенные в каталогах, таких как `/dev` (устройства) или через виртуальную файловую систему (например, сокеты).



## Сравнение Файловой Парадигмы с Альтернативами

| Характеристика | Файловая Парадигма (UNIX/Linux) | Объекты (Windows NT/BeOS) | Микроядра (QNX/MINIX) |
| :--- | :--- | :--- | :--- |
| **Универсальность** | **Высочайшая.** Единый интерфейс (поток байтов) для всего. | Средняя. Использует несколько интерфейсов (объектный хэндл, Реестр, файловый дескриптор). | Высокая. Единый интерфейс (передача сообщений/RPC) для взаимодействия. |
| **Производительность** | **Очень высокая.** Минимум накладных расходов. Прямой доступ к данным через ядро. | Средняя. Требуются вызовы методов, проверки безопасности объектов. | Ниже. Накладные расходы на постоянную передачу сообщений и переключение контекста между серверами. |
| **Безопасность/Стабильность** | Средняя. Сбой в ядре (например, в драйвере) **валит всю систему** (монолитное ядро). | Высокая. Строгие разрешения для доступа к объектам (более тонкая настройка безопасности). | **Высочайшая.** Драйверы и сервисы изолированы в пространстве пользователя. Сбой сервера не влияет на ядро. |
| **Расширяемость** | Отличная. Новое устройство просто добавляет новый "файл" в `/dev`. | Средняя. Требует регистрации новых типов объектов и их методов. | Отличная. Новый сервис/драйвер — это новый процесс (сервер), который общается через сообщения. |

### Преимущества Файловой Парадигмы UNIX

1.  **Простота и Элегантность (KISS Principle):**
    * Единый, унифицированный интерфейс (дескрипторы) позволяет программистам работать с диском, терминалом и сетью одинаково. Это снижает сложность разработки приложений.
    * Простые инструменты легко комбинируются (например, команда `ls` может быть перенаправлена в `grep`, а результат — в файл), так как все они работают с потоками байтов.
2.  **Высокая Производительность (Монолитное Ядро):**
    * Вся логика (файловая система, драйверы) находится внутри ядра (монолитная архитектура). Это означает **минимальное переключение контекста** при доступе к ресурсам, что критически важно для производительности.
3.  **Гибкость:**
    * Легко создавать новые виртуальные ресурсы (например, `/proc`, `/sys`) без изменения основного интерфейса.

### Недостатки Файловой Парадигмы UNIX

1.  **Риск для Стабильности (Монолитность):**
    * Главный недостаток: поскольку драйверы и сервисы работают в режиме ядра, ошибка в одном из них (например, дефектный драйвер Wi-Fi) может **непосредственно привести к падению всего ядра (Kernel Panic)**. В Микроядре такой сбой просто перезапускает сервер.
2.  **Недостаточность для Сложных Операций:**
    * Интерфейс "потока байтов" не всегда подходит для сложных, структурированных операций. Например, для получения информации о процессе (ID, потребление памяти) приходится "читать" файл (`/proc/PID/status`) и парсить текстовую информацию. В Объектной парадигме вы бы просто вызвали метод `Process.GetStatus()`, получив структурированные данные.
3.  **Проблемы с Конфигурацией (Разрозненность):**
    * Конфигурация системы часто разбросана по множеству текстовых файлов (`/etc/`, `/var/`). Изменения не всегда транзакционны, что может привести к нестабильному состоянию системы при ошибках или сбоях питания (в отличие от атомарного Реестра Windows).

Таким образом, выбор парадигмы — это всегда компромисс между **Производительностью и Простотой (UNIX)**, **Безопасностью и Модульностью (Микроядра)** и **Структурным Управлением (Объекты/Реестры)**.

### Резюме

Поэтому, хотя файловая парадигма **UNIX** очень элегантна и гибка, другие ОС предпочитают более структурированные, защищенные или специализированные методы организации:

| Парадигма | Принцип | Универсальный Интерфейс |
| :--- | :--- | :--- |
| **UNIX/Linux** | Все есть файл | Файл/Дескриптор (Набор байт) |
| **Windows NT** | Все есть объектный хэндл/запись реестра | Хэндл/Запись |
| **Микроядра** | Все есть сервис, взаимодействующий через сообщения | Сообщение/Порт |
